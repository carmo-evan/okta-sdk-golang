...
// APIClient manages communication with the Okta Management APIs API v3.0.0
...
func (c *APIClient) NewRequest(method string, URL string, body interface{}) (*http.Request, error) {
	...
	URL = c.cfg.Okta.Client.OrgUrl + URL
	...
	if c.cfg.Okta.Client.AuthorizationMode == "SSWS" {
		req.Header.Add("Authorization", "SSWS "+c.cfg.Okta.Client.Token)
	}

	if c.cfg.Okta.Client.AuthorizationMode == "Bearer" {
		req.Header.Add("Authorization", "Bearer "+c.cfg.Okta.Client.Token)
	}

	// TODO need to support private key
...
//	}
//	req.Header.Add("Accept", c.cfg.headerAccept)
//	if body != nil {
//		req.Header.Set("Content-Type", c.cfg.headerContentType)
//	}
...
func (c *APIClient) do(ctx context.Context, req *http.Request, v interface{})(*APIResponse, error){
	resp, err := c.doWithRetries(ctx, req)
	if err != nil {
		return nil, err
	}
	return buildResponse(resp, c, &v)
}
...
func (c *APIClient) doWithRetries(ctx context.Context, req *http.Request) (*http.Response, error) {
	...
	if c.cfg.Okta.Client.RequestTimeout > 0 {
		ctx, _ = context.WithTimeout(ctx, time.Second*time.Duration(c.cfg.Okta.Client.RequestTimeout))
	}
	bOff := &oktaBackoff{
		ctx:        ctx,
