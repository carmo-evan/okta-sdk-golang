...
> ListPushProviders200ResponseInner UpdatePushProvider(ctx, pushProviderId).PushProvider(pushProvider).Execute()
...
func main() {
    ...
    configuration := openapiclient.NewConfiguration()
    apiClient := openapiclient.NewAPIClient(configuration)
    resp, r, err := apiClient.PushProviderApi.UpdatePushProvider(context.Background(), pushProviderId).PushProvider(pushProvider).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `PushProviderApi.UpdatePushProvider``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `UpdatePushProvider`: ListPushProviders200ResponseInner
    fmt.Fprintf(os.Stdout, "Response from `PushProviderApi.UpdatePushProvider`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiUpdatePushProviderRequest struct via the builder pattern
